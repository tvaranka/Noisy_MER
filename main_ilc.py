    ...
    for group in df.groupby("subject"):
        ...
        for epoch in range(epochs):
            ...
            for batch in dataset_loader_train:
                ...
                losses = criterion2(outputs, labels_batch.long())
                _, predicted = torch.max(outputs, 1)
                threshold = losses.mean() + alpha * losses.std()
                labels_batch[losses > threshold] = predicted[losses > threshold].int()
                loss = criterion1(outputs, labels_batch.long())
                loss.backward()
                optimizer.step()
...
LOSO(uv_frames, df, epochs=200, lr=0.01, weight_decay=0.001,
     dropout=0.5, batch_size=128, ltm=False)
